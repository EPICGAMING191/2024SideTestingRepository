...
## What Is RoadRunner?
**RoadRunner** is a motion library that most teams use for autonomous movement. RoadRunner allows us to move in any direction and turn precisely without using advanced math. Before using RoadRunner, we need to tune RoadRunner to run accurately based on the specifics of our robot. To tune RoadRunner, we run some of the RoadRunner files in a specific order. These files will output numbers that we will input into a file in our project called the **config file**. A flowchart of the order of the programs that we need to run is available on the RoadRunner website. Once our tuning is complete, RoadRunner will be able to accurately move the robot in precise movements that are run by our autonomous program. RoadRunner is tuned differently depending on if your robot is using odometry wheels or not, so make sure to choose the correct path on the flowchart.
